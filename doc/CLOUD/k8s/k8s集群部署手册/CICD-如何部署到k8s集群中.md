...
pid_file='/var/run/confd_reload_cmd.pid'
...
do_start() {
  do_cleanup_undo
  /usr/local/bin/confd -log-level info -backend etcdv3 -node http://10.250.3.100:2379 -watch >>/data/server/k8s-deploy/logs/.confd.log 2>&1 &
}

do_stop() {
  kill $(ps -ef |grep '/usr/local/bin/confd -log-level info -backend etcdv3 -node http://10.250.3.100:2379 -watch' |grep -v grep |awk '{print $2}')
}

do_status() {
  ps -ef |grep '/usr/local/bin/confd -log-level info -backend etcdv3 -node http://10.250.3.100:2379 -watch' |grep -v grep
}
